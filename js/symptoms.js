...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const hospitalsData = [
        { 
            name: "City General Hospital", spec: "General / Emergency", dist: 1.2, rating: 4.8, address: "100 Med Center Dr, City", phone: "555-0101", emergency: true, coords: "40.7128,-74.0060",
            doctors: [
        ...
        { 
            name: "Heart & Neuro Center", spec: "Cardiology / Neurology", dist: 3.4, rating: 4.9, address: "400 Specialist Way, City", phone: "555-0202", emergency: true, coords: "40.7306,-73.9352",
            doctors: [
        ...
        { 
            name: "Community Care Clinic", spec: "Urgent Care", dist: 0.8, rating: 4.2, address: "50 Local Rd, Suburb", phone: "555-0303", emergency: false, coords: "40.7488,-73.9857",
            doctors: [
        ...
        { 
            name: "Mercy Trauma Center", spec: "Level 1 Trauma", dist: 5.1, rating: 4.6, address: "999 Rescue Blvd, City", phone: "555-9111", emergency: true, coords: "40.7580,-73.9855",
            doctors: [
    ...
    function renderAnalysis(symptoms, severity) {
        ...
        if (isEmergency) {
            levelDisplay.textContent = 'CRITICAL ALERT - IMMEDIATE ACTION REQUIRED';
            levelDisplay.classList.add('risk-danger');
            descDisplay.innerHTML = 'Our AI analysis has detected highly critical symptom signatures that correlate with severe medical events. <br><br><b>Please do safely seek emergency medical assistance immediately. Do not attempt to drive yourself.</b>';
            emergencyWarning.classList.remove('hidden');
            
            conditionsList.innerHTML = `
                <li style="margin-bottom:12px; line-height: 1.4;">
                    <strong style="color: #ef4444;">🔴 Suspected Deep Tissue / Cardiac Anomaly</strong><br>
                    <span style="font-size: 0.9em; opacity: 0.85;">Symptoms match critical cardiopulmonary distress markers. Immediate stabilization recommended.</span>
                </li>
                <li style="margin-bottom:12px; line-height: 1.4;">
                    <strong style="color: #ef4444;">🔴 Acute Respiratory / Vascular Event</strong><br>
                    <span style="font-size: 0.9em; opacity: 0.85;">High probability of severe systemic disruption requiring specialized emergent intervention.</span>
                </li>
            `;
        } else if (severity > 4) {
            levelDisplay.textContent = 'MODERATE INTERVENTION SUGGESTED';
            levelDisplay.classList.add('risk-medium');
            descDisplay.innerHTML = 'Our system has cross-referenced your inputs and indicates a potential moderate-grade medical condition. While not immediately life-threatening, clinical evaluation is strongly recommended within 24-48 hours.';
            emergencyWarning.classList.add('hidden');
            
            conditionsList.innerHTML = `
                <li style="margin-bottom:12px; line-height: 1.4;">
                    <strong style="color: #f59e0b;">🟠 Probable Viral/Bacterial Infection</strong><br>
                    <span style="font-size: 0.9em; opacity: 0.85;">Consistent with upper-respiratory or localized infectious profiles. Requires physician diagnosis.</span>
                </li>
                <li style="margin-bottom:12px; line-height: 1.4;">
                    <strong style="color: #f59e0b;">🟠 Moderate Inflammatory Response</strong><br>
                    <span style="font-size: 0.9em; opacity: 0.85;">Localized swelling or systemic immune reactions detected based on pain scale and symptom description.</span>
                </li>
            `;
        } else {
            levelDisplay.textContent = 'ROUTINE OBSERVATION';
            levelDisplay.classList.add('risk-low');
            descDisplay.innerHTML = 'Based on the inputted vectors, your clinical picture aligns with self-resolving or benign irregularities. Monitor closely, maintain proper hydration, and rest.';
            emergencyWarning.classList.add('hidden');
            
            conditionsList.innerHTML = `
                <li style="margin-bottom:12px; line-height: 1.4;">
                    <strong style="color: #22c55e;">🟢 Benign Systemic Fatigue</strong><br>
                    <span style="font-size: 0.9em; opacity: 0.85;">Matches profiles of standard sleep deficit or mild physiological stress.</span>
                </li>
                <li style="margin-bottom:12px; line-height: 1.4;">
                    <strong style="color: #22c55e;">🟢 Uncomplicated Seasonal Immunity Response</strong><br>
                    <span style="font-size: 0.9em; opacity: 0.85;">Symptoms resemble standard common cold or brief allergic reactive markers. Expected to clear shortly.</span>
                </li>
            `;
        }
    }
    ...
    findHospitalsBtn.addEventListener('click', () => {
        ...
        hospitalsData.forEach(h => {
            ...
            el.innerHTML = `
                <h5>${h.name} <span class="h-dist">${h.dist} km</span></h5>
                <p>${h.spec}</p>
                <div class="h-meta">
                    <span>⭐ ${h.rating}</span>
                </div>
    ...
    function showHospitalDetails(h) {
        ...
        document.getElementById('h-detail-name').textContent = h.name;
        document.getElementById('h-detail-spec').textContent = h.spec;
        document.getElementById('h-detail-dist').textContent = `${h.dist} km`;
        document.getElementById('h-detail-rating').textContent = `⭐ ${h.rating}`;
        document.getElementById('h-detail-address').textContent = h.address;
        document.getElementById('h-detail-phone').textContent = h.phone;
        document.getElementById('h-detail-emergency').textContent = h.emergency ? 'Yes - 24/7' : 'No - Clinic Hours';
        ...
        // Map Link Generation
        // In real app, we'd use current location or query
